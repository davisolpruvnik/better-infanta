  ...
  return (
    <div className="mb-8 justify-between items-start gap-6 flex flex-col lg:flex-row border-b border-gray-100 pb-6">
      <div className="flex-1 min-w-0 text-center lg:text-start">
        ...
        {doc.description && (
          <p className="text-lg text-gray-700/70 mt-2 leading-snug tracking-wide font-axis-subtitular-focus mx-auto lg:mx-0 break-words text-wrap hyphens-auto max-w-2xl md:max-w-3xl lg:max-w-3xl">
            {doc.description}
          </p>
        )}
      </div>
          ...
          <div className="flex items-center justify-center lg:justify-end gap-6 sm:gap-6 md:gap-8 lg:gap-8 w-full lg:w-auto">
            <div className="relative flex flex-col items-center lg:items-end text-center lg:text-end">
              <div className="flex flex-row items-center">
                <span className="block uppercase text-[14px] xs:text-[12px] font-axis-sng-indlab-header text-gray-600/80 tracking-widest">
                  Estimated Cost
                </span>
                {doc.feeDetails && (
                  <Popover.Root>
                    <Popover.Trigger
                      openOnHover
                      className="inline-flex items-center justify-center px-1 rounded-full text-burgundy-900/60 hover:text-burgundy-950 hover:bg-burgundy-50/50 transition-colors focus:outline-none cursor-pointer"
                    >
                      {renderIcon('tabler:help-circle', 'h-4 w-4')}
                    </Popover.Trigger>
                    <Popover.Portal>
                      <Popover.Positioner side="bottom" sideOffset={6}>
                        <Popover.Popup className="z-50 max-w-xs p-2.5 bg-white border border-gray-200 rounded-xl shadow-lg text-[10px] leading-relaxed text-gray-600 normal-case origin-[var(--transform-origin)] transition-all duration-200 ease-out data-[starting-style]:scale-90 data-[starting-style]:opacity-0 data-[ending-style]:scale-90 data-[ending-style]:opacity-0">
            ...
            <div className="relative flex flex-col items-center lg:items-end text-center lg:text-end">
              <span className="text-[14px] font-axis-sng-indlab-header text-gray-600/80 uppercase tracking-widest">
                Processing Time
              </span>
