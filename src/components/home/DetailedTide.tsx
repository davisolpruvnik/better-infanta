...
export default function TideCard({ className = '' }: { className?: string }) {
  ...
  const { extrema, highPeaks } = useMemo(() => {
    ...
    for (let i = 1; i < heights.length - 1; i++) {
      ...
      if (curr > prev && curr > next) {
        ...
        const exactHeight = curr - ((prev - next) ** 2) / (8 * (prev - 2 * curr + next));
    ...
    let peakCount = 0;
    const formattedEvents: ExtremaEvent[] = allEvents.map(e => {
      if (e.type === 'HIGH') {
        peakCount++;
    ...
    const peaksOnly = formattedEvents.filter(e => e.type === 'HIGH').slice(0, 2);

    return {
      extrema: formattedEvents.slice(0, 4),
      highPeaks: peaksOnly,
    };
  }, [hourlyData]);

  // 💡 2. Smooth Cubic Wave Geometry (Mirrors WeatherCard's smooth curve & scaling)
  const { linePath, points, zeroY, peakPoints } = useMemo(() => {
    if (!hourlyData || hourlyData.heights.length < 2) {
      return { linePath: '', points: [], zeroY: 50, peakPoints: [] };
    }
    ...
    // 💡 Map Y between 25 and 75 (leaves 25px top headroom for peak labels)
    ...
    // Locate the two high-tide crests
    const peaks: { pt: TidePoint; label: string }[] = [];
    for (let i = 1; i < calculatedPoints.length - 1; i++) {
      ...
      if (curr > prev && curr > next) {
        // Exact minute & height interpolation between hourly samples
        const delta = (prev - next) / (2 * (prev - 2 * curr + next));
        const exactHeight = curr - ((prev - next) ** 2) / (8 * (prev - 2 * curr + next));
        ...
        // 24-hour format (e.g. "04:18" or "16:42")
        const exact24Time = baseDate.toLocaleTimeString('en-GB', {
          hour: '2-digit',
          minute: '2-digit',
          hour12: false,
        });
        ...
        peaks.push({
          pt: calculatedPoints[i],
          label: `Peak ${peaks.length + 1}`,
          timeStr: exact24Time,
          height: Number(exactHeight.toFixed(2)),
        });
  ...
  return (
    <div className="w-full lg:flex-1 border-t-2 border-b-2 border-fantas-900/90 py-4 px-2 flex flex-col justify-between select-none bg-transparent text-fantas-900/90">
      {/* 📰 Broadsheet Masthead Header */}
      <div className="border-b border-fantas-900/90 pb-2.5 text-start">
        <div className="flex items-baseline justify-between">
          <span className="text-[11px] sm:text-xs uppercase tracking-wider font-axis-navbar-focus">
            Tidal Bulletin
          </span>
        </div>
        <h3 className="text-base sm:text-xl lg:text-xl font-axis-titular-focus uppercase tracking-wide mt-1">
      ...
      <div className="py-4 relative w-full my-2 sm:my-3">
          ...
          <div className="absolute inset-0 pointer-events-none">
            {peakPoints.map((item, idx) => (
              <div
                ...
                className="absolute"
                style={{
                  left: `${(item.pt.x / 500) * 100}%`,
                  top: `${(item.pt.y / 100) * 100}%`,
                }}
              >
                <div className="absolute -translate-x-1/2 bottom-2.5 flex flex-col items-center whitespace-nowrap leading-tight">
                  <span className="text-[14px] font-axis-plantao-num-focus text-fantas-900/90 tracking-wide proportional-nums">
                    {item.pt.height > 0 ? `+${item.pt.height.toFixed(2)}m` : `${item.pt.height.toFixed(2)}m`}
                  </span>
                ...
                <div className="absolute -translate-x-1/2 -translate-y-1/2 flex items-center justify-center">
                  <span className="size-2.5 rounded-full bg-white border-2 border-neutral-900 shadow-2xs" />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="w-full flex justify-between text-[11px] text-fantas-900/70 px-1 mt-2 border-t border-fantas-800/10 pt-1 font-axis-navbar-focus tracking-wide">
      ...
      {/* 📋 Daily Extrema Table */}
      <div className="pt-2 border-t border-fantas-900/40">
